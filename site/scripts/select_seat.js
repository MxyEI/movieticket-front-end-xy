...
$(document).ready(function () {
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              ...
              _context.next = 5;
              return get_movie_on_show();

            case 5:
              movie_on_show = _context.sent;
  ...
  var select_seat_part = function () {
    var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee4() {
      var seat_layout, unavailable;
      return regeneratorRuntime.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              _context4.next = 2;
              return get_seat_layout(cinemaHallId);

            case 2:
              seat_layout = _context4.sent;
              ...
              // 获取不可用座位信息
              _context4.next = 7;
              return get_unavailable(movieOnShowId);

            case 7:
              unavailable = _context4.sent;
              ...
              // 点击更改场次按钮，显示更改场次的框
              movie_info_change_show_time.onclick = function () {
                    ...
                    while (1) {
                      switch (_context2.prev = _context2.next) {
                        case 0:
                          ...
                          _context2.next = 5;
                          return get_day(showDate, cinemaId, movieId);
                        ...
                        case 17:
                          if (movie_info_change_show_time.innerHTML == '确定') {
                            // 隐藏更改场次弹框
                            hide_movie_info_change_show_time_dialog();
                          }
              ...
              // 点击某个场次进行场次变化
              movie_info_change_show_time_dialog.onclick = function () {
                var _ref4 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3(event) {
                  var movie_on_show, cinema_hall, _seat_layout, _unavailable;
                    ...
                    while (1) {
                      switch (_context3.prev = _context3.next) {
                        case 0:
                          ...
                          // 获取电影排期（根据Id）
                          _context3.next = 4;
                          return get_movie_on_show_by_id(changedMovieOnShowId);

                        case 4:
                          movie_on_show = _context3.sent;

                          changedCinemaHallId = movie_on_show.cinemaHallId;
                          // 获取影厅信息（不含座位布局）
                          _context3.next = 8;
                          return get_cinema_hall(changedCinemaHallId);

                        case 8:
                          cinema_hall = _context3.sent;

                          // 变化影厅名
                          movie_info_cinema_hall_name.innerHTML = cinema_hall.hallName;
                          // 根据收到的字符串生成大布局矩阵
                          _context3.next = 12;
                          return get_seat_layout(changedCinemaHallId);

                        case 12:
                          _seat_layout = _context3.sent;

                          initial_viaribles_with_seat_layout(_seat_layout);
                          show_select_seat_layout_matrix(_seat_layout, select_seat_layout_matrix, select_seat_row_count, select_seat_column_count);
                          // 将不可选座位的图标改为灰色
                          _context3.next = 17;
                          return get_unavailable(changedMovieOnShowId);

                        case 17:
                          _unavailable = _context3.sent;

                          show_unavailable_seats(_unavailable, select_seat_layout_matrix);
                          set_custom_scroll_bar();
                          select_seat.onclick = function (event) {
                            if (event.target.className == 'select_seat_seats_item select_seat_seats_item_avaliable' && event.target.id.slice(0, 12) == 'select_seat_') {
                              if (Object.keys(select_seats).length < 4) {
    ...
    return function select_seat_part() {
  ...
  var cinemaHallId = location.href.split('?')[1].split('&')[0].split('=')[1],
  ...
  function show_movie_info(movie_info, movie_on_show) {
    ...
    var month_tmp = showDate.split('-')[1],
    ...
    movie_info_price.innerHTML = '\uFFE5' + global_movie_price + ' \xD7 0\u5F20';
  ...
  function show_select_seat_layout_matrix(seat_layout, select_seat_layout_matrix, select_seat_row_count, select_seat_column_count) {
    // select_seat = document.getElementById("select_seat");
    var select_seat_numbers_fragment = document.createDocumentFragment();
    for (var i = 1; i <= select_seat_row_count; i++) {
      var select_seat_number = document.createElement('div');
    ...
    var select_seat_fragment = document.createDocumentFragment();

    for (var _i = 1; _i <= select_seat_row_count; _i++) {
      var tmpK = 1;
      var select_seat_row = document.createElement('div');
      ...
      var select_seat_seats = document.createElement('div');
      ...
      for (var j = 1; j <= select_seat_column_count; j++) {
        var select_seat_seats_item = document.createElement('div');
        select_seat_seats_item.className = 'select_seat_seats_item';
        select_seat_seats_item.id = 'select_seat_' + _i + '_' + j;
        if (select_seat_layout_matrix[_i - 1].slice(j - 1, j) == 1) {
          ...
          select_seat_seats_item.setAttribute('name', 'select_seat_' + _i + '_' + tmpK);
          tmpK++;
        }
  ...
  function show_unavailable_seats(unavailable, select_seat_layout_matrix) {
    for (var i = 0; i < unavailable.data.length; i++) {
      var select_seat_this_row_array = [],
          tmpObj = {};
      for (var k = 0; k < select_seat_layout_matrix[unavailable.data[i][0] - 1].length; k++) {
        select_seat_this_row_array.push(select_seat_layout_matrix[unavailable.data[i][0] - 1].slice(k, k + 1));
      }
      for (var j = 0; j < select_seat_this_row_array.length; j++) {
  ...
  function change_item_initial(event) {
    var lengthTmp = event.target.parentNode.childNodes.length;
  ...
  function show_movie_info_change_show_time_dialog_items(day, day_times) {
    var movie_info_change_show_time_dialog_item = document.createElement('div');
  ...
  function show_select_seats() {
    var lengthTmp = movie_info_seats.childNodes.length;
    for (var i = 0; i < lengthTmp; i++) {
      movie_info_seats.removeChild(movie_info_seats.childNodes[0]);
    }
    var arrTmp = Object.keys(select_seats),
        movie_info_seats_item_fragment = document.createDocumentFragment();
    for (var _i2 = 0; _i2 < arrTmp.length; _i2++) {
      var movie_info_seats_item = document.createElement('div');
      movie_info_seats_item.className = 'movie_info_seats_item';
      movie_info_seats_item.innerHTML = select_seats[arrTmp[_i2]];
    ...
    movie_info_price.innerHTML = '\uFFE5' + global_movie_price + ' \xD7 ' + arrTmp.length + '\u5F20';
  ...
  function sort_select_time_items() {
    var lengthTmp = movie_info_change_show_time_dialog.childNodes.length,
        objTmp = {};
    for (var i = 0; i < lengthTmp; i++) {
    ...
    for (var _i3 = 0; _i3 < lengthTmp; _i3++) {
      for (var j = 0; j < lengthTmp - 1; j++) {
        if (movie_info_change_show_time_dialog.childNodes[j].id == objTmp[arrTmp[_i3]]) {
  ...
  function clear_all_seats() {
    var lengthTmp = select_seat.childNodes.length;
    for (var i = 0; i < lengthTmp; i++) {
      select_seat.removeChild(select_seat.childNodes[0]);
    }
    select.removeChild(select_seat);
    var select_seat2 = document.createElement('div');
    ...
    for (var _i4 = 0; _i4 < lengthTmp; _i4++) {
  ...
  function getInfo(event) {
    ...
    for (var i = 0; i < seats.length; i++) {
      for (var j = 0; j < seats[i].innerHTML.length; j++) {
        if (seats[i].innerHTML[j] == '排') str_temp += '_';else if (seats[i].innerHTML[j] == '座') ;else if (seats[i].innerHTML[j] != '\ ') str_temp += seats[i].innerHTML[j];
